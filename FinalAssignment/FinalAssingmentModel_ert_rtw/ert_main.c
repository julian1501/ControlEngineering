...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    FinalAssingmentModel_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(FinalAssingmentModel_M) == (NULL)) &&
    !rtmGetStopRequested(FinalAssingmentModel_M);
  while (runModel) {
    ...
    if (rtmStepTask(FinalAssingmentModel_M, 2)
        ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(FinalAssingmentModel_M->extModeInfo, 3, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(FinalAssingmentModel_M, true);
      }

      if (rtmGetStopRequested(FinalAssingmentModel_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(FinalAssingmentModel_M->extModeInfo, 3, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(FinalAssingmentModel_M, true);
      }
    }

    FinalAssingmentModel_step(0);

    /* Get model outputs here */
    rtExtModeCheckEndTrigger();
    pthread_mutex_unlock(&rateTaskFcnRunningMutex[0]);
    stopRequested = !((rtmGetErrorStatus(FinalAssingmentModel_M) == (NULL)) &&
                      !rtmGetStopRequested(FinalAssingmentModel_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(FinalAssingmentModel_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  FinalAssingmentModel_terminate();
  rtExtModeShutdown(3);
  sem_post(&stopSem);
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  mwRaspiInit();
  MW_launchPyserver();
  rtmSetErrorStatus(FinalAssingmentModel_M, 0);
  rtExtModeParseArgs(argc, (const char_T **)argv, NULL);

  /* Initialize model */
  FinalAssingmentModel_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(FinalAssingmentModel_M));
  rtExtModeCheckInit(3);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(FinalAssingmentModel_M->extModeInfo, 3, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(FinalAssingmentModel_M, true);
    }
  }
